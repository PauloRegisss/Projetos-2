...
import { unstable_base64ToBytes as base64ToBytes, unstable_getErrorInfo as getErrorInfo } from '../../minimal/server.js';
import { INTERNAL_ServerRouter } from '../client.js';
import { getPathMapping } from '../isomorphic-utils/path-spec.js';
import { decodeSliceId, encodeRoutePath, pathnameToRoutePath } from '../isomorphic-utils/route-path.js';
import { getRouterPrefetchCode, setupRouterSearchCodecs } from './client-code.js';
...
const resolveInternalRoute = (location, base)=>{
...
export const createRequestHandler = ({ configRegistry, routeEntries, runHandled })=>{
        ...
        return runHandled(input.req, async ()=>{
            requestElementCacheInit ??= (async ()=>{
                ...
                if (cachedElementsMetadata) {
                    Object.entries(JSON.parse(cachedElementsMetadata)).forEach(([cacheId, str])=>{
            ...
            const getPath2moduleIds = async ()=>{
                if (!cachedPath2moduleIds) {
                    cachedPath2moduleIds = JSON.parse(await loadBuildMetadata(DEFINE_ROUTER_METADATA.path2moduleIds) || '{}');
                }
            ...
            const withRerender = async (fn)=>{
                ...
                const rerender = (rscPath, rscParams)=>{
                    ...
                    entriesPromise = Promise.all([
                        entriesPromise,
                        routeEntries.getEntriesForRoute(rscPath, rscParams, clientEtags, requestElementCache)
            ...
            const getEntriesForRedirect = async (location)=>{
                ...
                return runHandled(destination, ()=>routeEntries.getEntriesForRoute(encodeRoutePath(redirectRoute.path), new URLSearchParams({
                        query: redirectRoute.query
                    }), clientEtags, requestElementCache)).catch(()=>null);
            };
            const handleRscRequest = async ({ rscPath, rscParams })=>{
                ...
                try {
                    ...
                    if (info?.location && info.status !== 404) {
                        ...
                        return renderRsc(redirected.elements, {
                            etags: redirected.etags
                        });
                    }
                    if (info?.status !== 404) {
                        throw e;
                ...
                if (!entries && configRegistry.has404()) {
                    entries = await routeEntries.getEntriesForRoute(encodeRoutePath('/404'), rscParams, clientEtags, requestElementCache).catch((e)=>{
            ...
            const handleHttpRequest = async ({ pathname, req, tryAction })=>{
                ...
                if (pathConfigItem?.type === 'api') {
                    ...
                    return pathConfigItem.handler(apiReq, {
                        params
                    });
                }
                const renderPage = async (pathname, query, status = 200)=>{
                    ...
                    return renderHtml(await renderRsc(entries.elements, {
                        etags: entries.etags
                    }), html, {
                        rscPath,
                        formState,
