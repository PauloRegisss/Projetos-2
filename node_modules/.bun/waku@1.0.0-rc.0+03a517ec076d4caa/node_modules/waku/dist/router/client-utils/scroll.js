...
const decodeHash = (raw)=>raw.replace(/(?:%[0-9A-Fa-f]{2})+/g, (escapes)=>{
        try {
...
export const getHashElement = (hash)=>{
...
export const scrollToHash = (hash, behavior, scrollTopForMissingHash)=>{
    if (hash) {
...
export const shouldScrollByDefault = (url)=>pathnameToCurrentRoutePath(url.pathname) !== pathnameToCurrentRoutePath(window.location.pathname) || url.hash !== window.location.hash;
export const shouldScrollForRouteChange = (next, prev)=>next.path !== prev.path || next.hash !== prev.hash;
