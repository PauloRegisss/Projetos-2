...
import { Component, createContext, startTransition, use, useCallback, useContext, useEffect, useEffectEvent, useLayoutEffect, useMemo, useRef, useState, useTransition } from 'react';
import { preloadModule } from 'react-dom';
import { ETAG_ID_PREFIX } from '../lib/utils/etags.js';
import { Root_UNSTABLE as Root, Slot_UNSTABLE as Slot, unstable_addBase as addBase, unstable_fetchRsc as fetchRsc, unstable_getErrorInfo as getErrorInfo, unstable_isImmutableElement as isImmutableElement, unstable_registerCallServerElementsListener as registerCallServerElementsListener, unstable_registerRscReloadListener as registerRscReloadListener, unstable_removeBase as removeBase, useElementsPromise_UNSTABLE as useElementsPromise, useMergeElements_UNSTABLE as useMergeElements } from '../minimal/client.js';
import { decideFollow, isFollowable } from './client-utils/error-route.js';
import { createPrefetchManager } from './client-utils/prefetch-cache.js';
import { getRouteUrl, isSameRoute, isSameRscRoute, parseRoute } from './client-utils/route-url.js';
import { ROUTER_STATE_ID, canCommitInstantly, getRouteFromElements, getRouterState, getSettledRoute, has404FromElements, isStaticFromElements, makeRouterState, pinForSwr, resolveServerRedirect } from './client-utils/router-state.js';
import { scrollToHash, shouldScrollByDefault, shouldScrollForRouteChange } from './client-utils/scroll.js';
...
import { HAS404_ID, IS_STATIC_ID, ROUTE_ID, encodeRoutePath, encodeSliceId, getRouteSlotId, getSliceSlotId } from './isomorphic-utils/route-path.js';
import { getRouteSearchCodecId, isCodec } from './isomorphic-utils/search-codec-registry.js';
const commitHistory = (url, mode)=>{
...
const reloadWithUrl = (url)=>{
    window.history.pushState(window.history.state, '', url);
    window.location.reload();
};
const useRefetch = ()=>{
    const mergeElements = useMergeElements();
    return useCallback((rscPath, rscParams, options = {})=>{
        const { prefetched, unstable_overlay, unstable_swr, ...fetchOptions } = options;
        const elements = prefetched ? abortable(prefetched, options.signal) : fetchRsc(rscPath, rscParams, {
            ...fetchOptions,
            ...unstable_swr?.base ? {
                unstable_base: unstable_swr.base
            } : {}
        });
...
const fetchRouteElements = (rscPath, rscParams, { signal, prefetched, onBuildIdMismatch, base })=>{
    return prefetched ? abortable(prefetched, signal) : fetchRsc(rscPath, rscParams, {
        signal,
        onBuildIdMismatch,
        unstable_base: base
    });
...
const canPaintInstantOverlay = (follows, routeSlotId, resolvedElements, prefetchedElements)=>!follows && canCommitInstantly(routeSlotId, resolvedElements, prefetchedElements);
const dispatchChangeRoute = (changeRoute, route, options, startTransitionFn = startTransition)=>{
...
const resolveRouteHref = (to, resolveCodec)=>addBase(typeof to === 'string' ? to : buildRouteHref(to, resolveCodec), import.meta.env.WAKU_CONFIG_BASE_PATH);
/**
 ...
 */ export function useRouter() {
    ...
    const navigate = useCallback((history, to, options)=>{
    ...
    const push = useCallback((to, options)=>navigate('push', to, options), [
        navigate
    ]);
    const replace = useCallback((to, options)=>navigate('replace', to, options), [
        navigate
    ]);
    ...
    const prefetch = useCallback((to, options)=>{
        const url = new URL(resolveRouteHref(to, resolveCodec), window.location.href);
...
const prefetchIfNotCurrent = (router, resolvedTo, options)=>{
    if (!router) {
        return;
    }
...
const usePrefetchOnView = (ref, router, resolvedTo, options)=>{
...
const FollowError = ({ error, has404, reset, fail })=>{
    ...
    const dispatchedRef = useRef(undefined);
    useEffect(()=>{
        ...
        if (routePath !== caughtPath || routeQuery !== caughtQuery || routeHash !== caughtHash) {
            reset();
            return;
        }
        const dispatched = dispatchedRef.current;
        if (dispatched && routerState && routerState !== dispatched.from) {
            const sameRequest = routerState.requested[0] === dispatched.route.path && routerState.requested[1] === dispatched.route.query;
            const followCompleted = sameRequest ? dispatched.route.path === routePath : routerState.url === dispatched.url;
            if (followCompleted) {
                reset();
            } else {
                fail(error, new Error('detected a navigation loop', {
                    cause: error
                }));
            }
        }
    ...
    const followCaughtError = useEffectEvent(()=>{
        ...
        const stateUrl = routerState ? new URL(routerState.url, window.location.href) : new URL(window.location.href);
        ...
        const decision = decideFollow(error, {
            route: caught,
            url: stateUrl,
            follows
        }, {
        ...
        startTransition(()=>{
            changeRoute(target, {
                shouldScroll: routerState ? routerState.scroll !== null : target.path !== caught.path,
                history: 'replace',
                url,
                refetch: true,
                follows: nextFollows
            }).catch((err)=>{
                const rejected = decideFollow(err, {
                    route: target,
                    url,
                    follows: nextFollows
                }, {
    ...
    return info?.status === 404 && !has404 ? /*#__PURE__*/ _jsx("h1", {
        children: "Not Found"
    }) : null;
};
class CustomErrorHandler extends Component {
    ...
    fail(original, error) {
        this.setState((state)=>state.error === original ? {
                error
            } : null);
    }
    render() {
        ...
        if (error !== null) {
            if (isFollowable(error)) {
                return /*#__PURE__*/ _jsx(FollowError, {
                    error: error,
...
const preloadRouteModules = (path)=>{
...
const fetchSlice = (id, mergeElements, fetchingSlices, options)=>{
 ...
 */ export function Slice({ id, children, ...props }) {
    ...
    const needsToFetchSlice = props.lazy && (!(slotId in elements) || !isImmutableElement(elements, slotId));
    useEffect(()=>{
        if (props.lazy) {
            lazySliceIds.add(id);
        }
...
const InnerRouter = ({ fallbackRoute, routeInterceptor })=>{
    const elementsPromise = useElementsPromise();
    const elements = use(elementsPromise);
    ...
    const destination = useMemo(()=>routerState && resolveServerRedirect(elements, routerState, initialRoute.path), [
        elements,
        routerState,
        initialRoute
    ]);
    ...
    useLayoutEffect(()=>{
        ...
        commitHistory(new URL(destinationHref), applied ? 'replace' : routerState.history);
    ...
    useEffect(()=>{
        if (import.meta.hot) {
            const refetchRouteOnHmr = ()=>{
                ...
                const settledRoute = getSettledRoute(resolvedElementsRef.current, routeFallback);
                startTransition(()=>{
                    // the reload clears the set, so the response has to teach it again
                    void refetch(encodeRoutePath(settledRoute.path), createRscParams(settledRoute.query)).then(addToStaticPathSet, ()=>{});
    ...
    const changeRoute = useCallback(async function changeRoute(nextRoute, options) {
        const settledRoute = getSettledRoute(resolvedElementsRef.current, routeFallback);
        const shouldRefetch = options.refetch ?? !isSameRscRoute(nextRoute, settledRoute);
        ...
        if (options.pendingTransition && shouldRefetch && !staticPathSetRef.current.has(nextRoute.path) && !canPaintInstantOverlay(options.follows ?? 0, getRouteSlotId(nextRoute.path), resolvedElementsRef.current, prefetchManagerRef.current.getElements(encodeRoutePath(nextRoute.path)))) {
        ...
        const requestedPathChanged = initialAttempt.route.path !== settledRoute.path;
        const makeStateForAttempt = (attempt, history)=>makeRouterState(attempt.route, attempt.url, {
                history,
                scroll: options.shouldScroll,
                pathChanged: requestedPathChanged || attempt.route.path !== settledRoute.path,
        ...
        const commit = (state, update, transition)=>{
            const callback = ()=>{
        ...
        const commitRoute = (route, state, transition)=>{
            commit(state, ()=>{
                void mergeElements({
                    [ROUTE_ID]: [
                        route.path,
        ...
        if (staticPathSetRef.current.has(nextRoute.path) || !shouldRefetch) {
            commitRoute(nextRoute, makeStateForAttempt(initialAttempt, options.history), options.instant ? undefined : options.startTransition);
            return;
        }
        const base = resolvedElementsRef.current;
        const fetchRoute = async (attempt, history, restoreBase)=>{
            if (attempt.follows > 0 && staticPathSetRef.current.has(attempt.route.path)) {
                return {
                    type: 'reused',
                    attempt,
                    history
            ...
            const instant = !!options.instant && canPaintInstantOverlay(attempt.follows, getRouteSlotId(attempt.route.path), resolvedElementsRef.current, prefetchedElements);
            const rscParams = createRscParams(attempt.route.query);
            let elements;
            try {
                elements = await (instant ? refetch(rscPath, rscParams, {
                    ...
                    unstable_swr: {
                        ...
                        ...prefetchedElements ? {
                            base: prefetchedElements
                        } : {}
                    },
                    onBuildIdMismatch: ()=>reloadWithUrl(attempt.url),
                    ...cached ? {
                        prefetched: cached.promise
                    } : {}
                ...
                if (// A render-time follow may be retrying the route whose slot threw.
                initialAttempt.follows === 0 && isSameRscRoute(decision.target, attempt.route) && isSameRscRoute(decision.target, settledRoute)) {
                    return {
                        type: 'reused',
                        attempt: nextAttempt,
                        history: nextHistory
        ...
        if (outcome.type === 'reused') {
            commitRoute(outcome.attempt.route, makeStateForAttempt(outcome.attempt, outcome.history), options.startTransition || startTransition);
            return;
        }
        ...
        const destination = resolveServerRedirect(elements, makeStateForAttempt(attempt, outcome.history), attempt.route.path);
        const finalState = makeRouterState(destination.route, destination.url, {
            history: outcome.history,
            scroll: options.shouldScroll,
            pathChanged: requestedPathChanged || destination.route.path !== settledRoute.path,
            follows: attempt.follows
        });
        commit(finalState, ()=>{
            const current = resolvedElementsRef.current;
            const update = {};
            const responseRoute = getRouteFromElements(elements) ?? destination.route;
            ...
            for (const [key, value] of Object.entries(elements)){
                if (rscRouteChanged && (key === routeSlotId || key === routeEtagId) || Object.hasOwn(current, key) === Object.hasOwn(base, key) && current[key] === base[key]) {
                    update[key] = value;
                }
            }
            Object.assign(update, {
                ...ROUTE_ID in elements ? {
                    [ROUTE_ID]: elements[ROUTE_ID]
                } : {},
    ...
    const changeRouteFromServer = useCallback(async (routeData, isStatic)=>{
        if (!routeData) {
            return;
        }
        const [path, query] = routeData;
        const settledRoute = getSettledRoute(resolvedElementsRef.current, routeFallback);
        if (settledRoute.path === path && (isStatic || settledRoute.query === query)) {
            return;
        }
        const route = {
            path,
    ...
    const prefetchRoute = useCallback((route, options)=>{
        ...
        const prefetchManager = prefetchManagerRef.current;
        prefetchManager.prefetch(rscPath, route.query, (base, invalidate)=>fetchRsc(rscPath, createRscParams(route.query), {
                ...base ? {
                    unstable_base: base
                } : {},
                onBuildIdMismatch: ()=>{
        ...
        const callback = ()=>{
            ...
            startTransition(()=>{
                changeRoute(nextRoute, {
                    shouldScroll: shouldScrollForRouteChange(nextRoute, getSettledRoute(resolvedElementsRef.current, routeFallback)),
                    history: null,
                    // keep the url it moved to; an interceptor rewrite needs a new one
                    url: isSameRoute(nextRoute, popped) ? new URL(window.location.href) : getRouteUrl(nextRoute)
                }).catch((err)=>{
                    if (!isFollowable(err)) {
    ...
    const routeElement = navigationError ? /*#__PURE__*/ _jsx(ThrowError, {
        error: navigationError.error
    }) : /*#__PURE__*/ _jsx(Slot, {
        id: getRouteSlotId(currentRoute.path)
    });
 ...
 * browser location.
 */ export function Router({ initialRoute = parseRoute(new URL(window.location.href)), unstable_routeInterceptor }) {
