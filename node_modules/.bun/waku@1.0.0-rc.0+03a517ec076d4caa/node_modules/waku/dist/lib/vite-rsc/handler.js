import { createTemporaryReferenceSet, decodeAction, decodeFormState, decodeReply, loadServerAction, renderToReadableStream } from '@vitejs/plugin-rsc/rsc/server';
...
const toProcessRequest = (handleRequest)=>async (req)=>{
        ...
        const renderUtils = createRenderUtils(temporaryReferences, renderToReadableStream, loadSsrEntryModule, import.meta.env.WAKU_BUILD_ID ?? '', debugChannel, debugId);
        let res;
        try {
            ...
            const documentLocation = info?.location ? resolveRedirectLocation(info.location, req.url, config.basePath) : undefined;
            // a document request is a real navigation, so it keeps the 3xx
            if (documentLocation && input.type !== 'http') {
                return new Response(await renderUtils.renderRsc({}, {
                    documentLocation
...
const toProcessBuild = (handleBuild)=>async ({ emitFile, unstable_registerPrunableFile })=>{
        const renderUtils = createRenderUtils(undefined, renderToReadableStream, loadSsrEntryModule, import.meta.env.WAKU_BUILD_ID ?? '');
        let fallbackHtml;
