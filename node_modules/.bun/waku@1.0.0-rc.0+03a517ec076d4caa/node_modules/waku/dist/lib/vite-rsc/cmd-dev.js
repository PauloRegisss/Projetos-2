...
loadDotEnv();
function createServerRestartHandler(strictPort) {
    ...
    return async (host, port, server)=>{
        ...
        try {
            ...
            const newServer = await startDevServer(host, port, freshConfig, strictPort, true);
            if (previousUrls) {
...
async function startDevServer(host, port, config, strictPort, isRestart) {
    if (isRestart) {
        loadDotEnv();
    }
